...
model = CNNRNNModel(input_channels=1, hidden_dim=1024, output_dim=3)
model.to(device)

# model.load_state_dict(torch.load("../data/models/model_bitmap_512notes200epochs.pth", map_location=device, weights_only=True))
model.load_state_dict(torch.load("../model_bitmap_other_loss.pth", map_location=device, weights_only=True))
model.eval()
