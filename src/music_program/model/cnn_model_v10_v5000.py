...
class MusicModel(nn.Module):
    def __init__(self, features_number, max_series_len):
        ...
        out_chanels = features_number
        self.conv1 = nn.Conv2d(in_channels=1, out_channels=out_chanels, kernel_size=7, stride=3, padding=1)

        self.bn1 = nn.BatchNorm2d(out_chanels)

        out_chanels *= 2
        self.conv2 = nn.Conv2d(in_channels=int(out_chanels / 2), out_channels=out_chanels, kernel_size=3, stride=1,
                           padding=1)
        self.bn2 = nn.BatchNorm2d(out_chanels)

        out_chanels *= 2
        self.conv3 = nn.Conv2d(in_channels=int(out_chanels / 2), out_channels=out_chanels, kernel_size=3, stride=1,
                           padding=1)
        self.bn3 = nn.BatchNorm2d(out_chanels)
