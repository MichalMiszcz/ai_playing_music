...
class CNNRNNModel(nn.Module):
    def __init__(self, input_channels=1, hidden_dim=1024, output_dim=3, rnn_layers=3, max_seq_len=100):
        ...
        # self.cnn.conv1 = nn.Conv2d(input_channels, 64, kernel_size=(32, 12), stride=(2, 1), padding=0, bias=False) #bias False, bo Conv ma BatchNorm
        self.cnn.conv1 = nn.Conv2d(input_channels, 64, kernel_size=14, stride=4, padding=6, bias=False)
        ...
        self.rnn = nn.LSTM(input_size=output_dim + hidden_dim, hidden_size=hidden_dim, num_layers=rnn_layers,
    ...
    def forward(self, x, target=None, teacher_ratio=None):
        ...
        if target is not None and teacher_ratio is not None and use_teacher_learning <= teacher_ratio:
            ...
            output, _ = self.rnn(rnn_input, (h0, c0))
