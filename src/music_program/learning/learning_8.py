...
def train_model(model, dataloader, val_dataloader, epochs=50, device=device, learning_rate=0.0005, weight_decay=0.00001,
    ...
    for epoch in range(epochs):
        ...
        if epoch > teacher_epochs:
            teacher_ratio = max(0.2, (0.8 - (epoch / epochs)) * 1.1)
        else:
            teacher_ratio = 1.0
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), f'src/model_best_index_v{version_name}.pth')
            print(f"Model saved as 'src/model_best_index_v{version_name}.pth'")
        else:
            patience_counter += 1
...
def generate_chart(data, title):
    ...
    plt.tight_layout()
    # plt.show()
    plt.savefig(f'src/plots/{title} CNNRNN_v{version_name}.png')
...
if __name__ == "__main__":
    ...
    learning_data, learning_data_val = train_model(model, dataloader, val_dataloader, epochs=epochs, device=device,
                                                   ...
                                                   lr_patience=lr_patience, es_patience=es_patience, teacher_epochs=teacher_epochs)
